...
import { registerCitizen, loginCitizen } from '@/lib/citizen-auth'
...
export function CitizenAuth({ onAuthenticated }: CitizenAuthProps) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    const result = loginCitizen(loginEmail, loginPassword)
    setLoading(false)
    if (result.ok) {
      onAuthenticated()
    } else {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    await new Promise((r) => setTimeout(r, 300))
    const result = registerCitizen(regEmail, regPassword, regName)
    setLoading(false)
    if (result.ok) {
      onAuthenticated()
    } else {
        ...
        <div className="w-full max-w-md">
          ...
          {tab === 'login' ? (
            ...
            <form onSubmit={handleRegister} className="space-y-5">
              ...
              <button
                type="submit"
                disabled={!regName || !regEmail || !regPassword || !regConfirm || loading}
                className="w-full h-11 bg-primary-600 text-white rounded-xl font-medium text-sm hover:bg-primary-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 flex items-center justify-center gap-2 group"
              >
                {loading ? (
                  <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
