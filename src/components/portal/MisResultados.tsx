...
const STATUS_CONFIG: Record<string, { label: string; color: string; icon: typeof Clock }> = {
  ...
  'tipo-seleccionado': { label: 'Tipo seleccionado', color: 'bg-primary-50 text-primary-700', icon: Clock },
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
      ...
      <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        ...
        <Link
          href={
            // Paso 5 (Simulador) -> confirmacion necesita ?id=; el resto del
            // STEP_ROUTES no lleva trámite específico.
            Number(tramite.currentStep) === 5
              ? `/portal/confirmacion?id=${tramite.id}`
              : STEP_ROUTES[tramite.currentStep] || '/portal'
          }
