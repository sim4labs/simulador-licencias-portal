'use client'

import Link from 'next/link'
import { Bus, Truck, Car, Bike, Ambulance, CircleDashed, Settings, ArrowRight, ExternalLink } from 'lucide-react'
import { cn } from '@/lib/utils'
import type { Device } from '@/lib/iot-api'
import { Button } from '@/components/ui/Button'

interface VehicleConfig {
  icon: typeof Bus
  label: string
  color: string       // border-l color
  iconBg: string      // online icon background
  iconColor: string   // online icon color
}

export const vehicleConfig: Record<string, VehicleConfig> = {
  passenger_bus: {
    icon: Bus,
    label: 'Camión Pasajeros',
    color: 'border-l-blue-500',
    iconBg: 'bg-blue-100',
    iconColor: 'text-blue-600',
  },
  cargo_truck: {
    icon: Truck,
    label: 'Camión Carga',
    color: 'border-l-amber-500',
    iconBg: 'bg-amber-100',
    iconColor: 'text-amber-600',
  },
  car: {
    icon: Car,
    label: 'Automóvil',
    color: 'border-l-emerald-500',
    iconBg: 'bg-emerald-100',
    iconColor: 'text-emerald-600',
  },
  motorcycle: {
    icon: Bike,
    label: 'Motocicleta',
    color: 'border-l-primary-500',
    iconBg: 'bg-primary-100',
    iconColor: 'text-primary-600',
  },
  emergency_vehicle: {
    icon: Ambulance,
    label: 'Vehículo de Emergencia',
    color: 'border-l-red-500',
    iconBg: 'bg-red-100',
    iconColor: 'text-red-600',
  },
}

export const spareConfig: VehicleConfig = {
  icon: CircleDashed,
  label: 'Sin asignar',
  color: '',
  iconBg: 'bg-gray-100',
  iconColor: 'text-gray-400',
}

function getRssiLabel(rssi: number): { text: string; color: string } {
  if (rssi >= -50) return { text: 'Excelente', color: 'text-emerald-600' }
  if (rssi >= -70) return { text: 'Buena', color: 'text-amber-600' }
  return { text: 'Débil', color: 'text-red-600' }
}

interface SimulatorCardProps {
  device: Device
  onEdit: (device: Device) => void
}

export function SimulatorCard({ device, onEdit }: SimulatorCardProps) {
  const isSpare = !device.vehicleType
  const config = device.vehicleType ? vehicleConfig[device.vehicleType] || spareConfig : spareConfig
  const Icon = config.icon

  // Online/offline es la jerarquía visual principal
  const cardClasses = device.online
    ? `bg-white shadow-lg ring-1 ring-gray-200 border-l-4 ${isSpare ? 'border-l-emerald-400' : config.color}`
    : 'bg-gray-50/70 border border-dashed border-gray-200'

  return (
    <div className={cn('rounded-lg flex flex-col', cardClasses)}>
      {/* Encabezado */}
      <div className="p-4 pb-3">
        <div className="flex items-start gap-3">
          <div className={cn(
            'flex-shrink-0 w-11 h-11 rounded-full flex items-center justify-center',
            device.online ? (isSpare ? 'bg-emerald-100' : config.iconBg) : 'bg-gray-100'
          )}>
            <Icon className={cn(
              'h-5 w-5',
              device.online ? (isSpare ? 'text-emerald-600' : config.iconColor) : 'text-gray-300'
            )} />
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between">
              <h3 className={cn(
                'font-semibold truncate',
                device.online ? 'text-gray-900' : 'text-gray-400'
              )}>
                {device.nickname || device.thingName}
              </h3>
              <span className="flex items-center gap-1.5 ml-2 flex-shrink-0">
                <span
                  className={cn(
                    'w-2.5 h-2.5 rounded-full',
                    device.online
                      ? 'bg-emerald-500 animate-pulse'
                      : 'bg-gray-300'
                  )}
                />
                <span className={cn('text-xs font-medium', device.online ? 'text-emerald-600' : 'text-gray-400')}>
                  {device.online ? 'En línea' : 'Fuera de línea'}
                </span>
              </span>
            </div>
            {isSpare ? (
              <p className={cn('text-sm italic', device.online ? 'text-gray-400' : 'text-gray-300')}>Sin asignar</p>
            ) : (
              <p className={cn('text-sm', device.online ? 'text-gray-500' : 'text-gray-400')}>{config.label}</p>
            )}
            {device.serialNumber && (
              <p className="text-xs text-gray-400 font-mono mt-0.5">S/N: {device.serialNumber}</p>
            )}
          </div>
        </div>
      </div>

      {/* Datos técnicos */}
      <div className={cn(
        'border-t border-gray-100 px-4 py-3 grid grid-cols-2 gap-y-2 gap-x-4 text-sm',
        !device.online && 'opacity-50'
      )}>
        <div>
          <span className="text-gray-400 text-xs">FW</span>
          <p className="font-mono text-gray-700 text-xs">{device.firmwareVersion || '—'}</p>
        </div>
        <div>
          <span className="text-gray-400 text-xs">RSSI</span>
          {device.rssi != null ? (
            <p className="text-xs">
              <span className="font-mono text-gray-700">{device.rssi} dBm</span>
              {' '}
              <span className={cn('font-medium', getRssiLabel(device.rssi).color)}>
                [{getRssiLabel(device.rssi).text}]
              </span>
            </p>
          ) : (
            <p className="text-xs text-gray-400">—</p>
          )}
        </div>
        <div>
          <span className="text-gray-400 text-xs">IP</span>
          {device.ip ? (
            <a
              href={`http://${device.ip}`}
              target="_blank"
              rel="noopener noreferrer"
              className="font-mono text-xs text-blue-600 hover:text-blue-800 hover:underline block"
            >
              {device.ip}
            </a>
          ) : (
            <p className="font-mono text-gray-700 text-xs">—</p>
          )}
        </div>
        <div>
          <span className="text-gray-400 text-xs">Última</span>
          <p className="text-gray-700 text-xs">
            {device.lastUpdate
              ? new Date(device.lastUpdate * 1000).toLocaleString('es-MX', {
                  day: 'numeric',
                  month: 'short',
                  hour: '2-digit',
                  minute: '2-digit',
                })
              : '—'}
          </p>
        </div>
      </div>

      {/* Acciones */}
      <div className="border-t border-gray-100 px-4 py-3 flex items-center justify-between mt-auto">
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={() => onEdit(device)}>
            <Settings className="h-3.5 w-3.5 mr-1.5" />
            {isSpare ? 'Asignar' : 'Configurar'}
          </Button>
          {device.online && device.ip && (
            <a href={`http://${device.ip}`} target="_blank" rel="noopener noreferrer">
              <Button variant="outline" size="sm">
                <ExternalLink className="h-3.5 w-3.5 mr-1.5" />
                Panel local
              </Button>
            </a>
          )}
        </div>
        <Link href={`/admin/iot/${device.thingName}`}>
          <Button variant="ghost" size="sm">
            Ver detalle
            <ArrowRight className="h-3.5 w-3.5 ml-1" />
          </Button>
        </Link>
      </div>
    </div>
  )
}
