...
export default function ResultadosPage() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    // Search by tramite ID first
    const tramites = getAllTramites()
    let found = tramites.find((t) => t.id.toUpperCase() === query)

    // Fallback: search by appointment code
    if (!found) {
      found = tramites.find((t) => t.appointment?.code.toUpperCase() === query)
    }

    // Fallback: search in legacy appointments
    if (!found) {
      const appointments = JSON.parse(localStorage.getItem('appointments') || '[]')
      const legacyAppt = appointments.find(
        (a: { code: string; tramiteId?: string }) => a.code.toUpperCase() === query
      )
      if (legacyAppt?.tramiteId) {
        found = tramites.find((t) => t.id === legacyAppt.tramiteId)
      }
    }

    if (found) {
      // Simulate simulator result for past appointments
      if (found.appointment && !found.simulatorResult) {
        const appointmentDate = new Date(found.appointment.date)
        if (appointmentDate < new Date()) {
          ...
          found = {
            ...
            simulatorResult: {
              ...
              completedAt: found.appointment.date,
            },
            status: passed ? 'finalizado' : 'simulador-completado',
          }
        }
      }
          ...
          {searchResult === 'not_found' && (
            <Card padding="lg" className="text-center">
              ...
              <Button asChild variant="outline">
                <Link href="/solicitud">Iniciar nuevo trámite</Link>
              </Button>
            </Card>
          )}
            ...
            <div className="space-y-6">
              ...
              <Card padding="lg">
                ...
                <div className="space-y-3">
                  {[
                    { num: 1, label: 'Solicitud', icon: FileText },
                    { num: 2, label: 'Tipo de Licencia', icon: Car },
